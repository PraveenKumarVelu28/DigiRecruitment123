...
@Component({
  selector: 'app-booked-transport-request-for-receptionist',
  templateUrl: './booked-transport-request-for-receptionist.component.html',
  styleUrls: ['./booked-transport-request-for-receptionist.component.css']
})
export class BookedTransportRequestForReceptionistComponent implements OnInit {
  search: any;
  DDate = new Date();
  RDate = new Date();
  ...
  public GetTransportationRequestsForReceptionist(DDate,RDate) {
    ...
    this.DigiOfficeService.GetTransportationRequestsForReceptionist(DDate,RDate).subscribe(res => {
      ...
      this.filteredList = this.transportationRequestsList.filter(x=>x.status=='Booked');
  ...
  public CompletedTransportationRequests(id) {
    this.DigiOfficeService.CompletedTransportationRequests(id).subscribe(
      data => {
        
        Swal.fire('completed Transport Request.');
        this.ngOnInit();
      }, error => {
      }
    )
