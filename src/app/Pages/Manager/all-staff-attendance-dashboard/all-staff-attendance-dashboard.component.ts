...
@Component({
  selector: 'app-all-staff-attendance-dashboard',
  templateUrl: './all-staff-attendance-dashboard.component.html',
  styleUrls: ['./all-staff-attendance-dashboard.component.css']
})
export class AllStaffAttendanceDashboardComponent implements OnInit {
  ...
  public GetAllAttendance(SDate, EDate){
    debugger
    this.DigiOfficeService.GetAllAttendance(SDate, EDate).subscribe(res=>{
      
      this.allStaffAttendanceList = res.filter(x=>x.supervisor==this.staffID);
    }
  )}
