...
@Component({
  selector: 'app-reject-locator-dashboard-for-manager',
  templateUrl: './reject-locator-dashboard-for-manager.component.html',
  styleUrls: ['./reject-locator-dashboard-for-manager.component.css']
})
export class RejectLocatorDashboardForManagerComponent implements OnInit {
  ...
  public GetMyLocatorRequests(managerID,SDate,EDate) {
    ...
    this.DigiOfficeService.GetLocatorRequestsReport(managerID,SDate,EDate).subscribe(res => {
      ...
      this.filterLocatorRequestsList = this.LocatorRequestsList.filter(x=>x.supervisorID==this.managerID && x.status==" Reject ");
    });
  }
  // public GetMyLocatorRequests(UserID,SDate,EDate) {
    
  //   this.DigiOfficeService.GetMyLocatorRequests(UserID,SDate,EDate).subscribe(res => {
      
  //     this.LocatorRequestsList = res;
  //     this.filterLocatorRequestsList = this.LocatorRequestsList.filter(x=>x.status=='Reject');
  //   });
  // }
