...
@Component({
  selector: 'app-all-staff-approved-leave',
  templateUrl: './all-staff-approved-leave.component.html',
  styleUrls: ['./all-staff-approved-leave.component.css']
})
export class AllStaffApprovedLeaveComponent implements OnInit {
  ...
  public GetStaffLeaves(Sdate,Edate) {
    ...
    this.DigiOfficeService.GetAllStaffLeavesForReceptionist(Sdate,Edate).subscribe(res => {
      ...
      this.appliedleaveList = res.filter(x => x.status == 'Approved' && x.staffRoleType != 9 && x.supervisor != this.hrid);
