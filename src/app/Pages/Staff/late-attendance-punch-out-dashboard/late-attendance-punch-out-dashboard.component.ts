...
@Component({
  selector: 'app-late-attendance-punch-out-dashboard',
  templateUrl: './late-attendance-punch-out-dashboard.component.html',
  styleUrls: ['./late-attendance-punch-out-dashboard.component.css']
})
export class LateAttendancePunchOutDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.GetAllAttendanceLatePunchout(this.todaydate, this.enddate);
    this.GetAllAbsetees();
  }
  ...
  public GetAllAttendanceLatePunchout(SDate, EDate){
    debugger
    this.DigiOfficeService.GetAllAttendanceLatePunchout(SDate, EDate).subscribe(data=>{
      
      this.latepunchOutList = data
      this.signInDate=this.latepunchOutList[0].signinDate;
       this.count=this.latepunchOutList.length;
    }
  )}
