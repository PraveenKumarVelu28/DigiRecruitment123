...
@Component({
  selector: 'app-late-attendance-dashboard',
  templateUrl: './late-attendance-dashboard.component.html',
  styleUrls: ['./late-attendance-dashboard.component.css']
})
export class LateAttendanceDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.GetAllAttendanceLatePunchin(this.todaydate, this.enddate);
    this.GetAllAbsetees();
  }
  ...
  public GetAllAttendanceLatePunchin(SDate, EDate){
    debugger
    this.DigiOfficeService.GetAllAttendanceLatePunchin(SDate, EDate).subscribe(data=>{
      
      this.latepunchinlist = data
      this.signInDate=this.latepunchinlist[0].signinDate;
       this.count=this.latepunchinlist.length;
    }
  )}
