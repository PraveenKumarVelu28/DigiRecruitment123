...
@Component({
  selector: 'app-approver3ratingpage',
  templateUrl: './approver3ratingpage.component.html',
  styleUrls: ['./approver3ratingpage.component.css']
})
export class Approver3ratingpageComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.fmsservice.GetConductappraisalStaffList().subscribe(
      res => {
        ...
        this.staffscore = this.StaffAppraisalList[0].selfScores
        this.staffcomments = this.StaffAppraisalList[0].selfComments

        this.managerscore = this.StaffAppraisalList[0].groupHeadScores
        this.managercomments = this.StaffAppraisalList[0].groupHeadComments

        this.cioscores = this.StaffAppraisalList[0].cioScores
        this.ciocomments = this.StaffAppraisalList[0].cioComments
  ...
  public insertdetails1() {
    ...
    for (this.q = 0; this.q < this.appraisalList.length; this.q++) {
      var entity = {
        ...
        'CEOScores': this.appraisalList[this.q].SelfScores,
        'CEOComments': this.appraisalList[this.q].SelfComments
      }
      this.fmsservice.UpdateCEOStaffScores(entity).subscribe(data => {
        debugger
        if (data != undefined && this.q == this.appraisalList.length) {
          Swal.fire("Appraisal Saved Successfully");
          this.router.navigate(['Approver3ratingpagedashboard']);
