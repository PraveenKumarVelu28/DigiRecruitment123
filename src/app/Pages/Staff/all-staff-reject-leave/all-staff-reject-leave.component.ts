...
@Component({
  selector: 'app-all-staff-reject-leave',
  templateUrl: './all-staff-reject-leave.component.html',
  styleUrls: ['./all-staff-reject-leave.component.css']
})
export class AllStaffRejectLeaveComponent implements OnInit {
  ...
  public GetStaffLeaves(Sdate,Edate) {
    ...
    this.DigiOfficeService.GetAllStaffLeavesForReceptionist(Sdate,Edate).subscribe(res => {
      ...
      this.appliedleaveList = res.filter(x => x.status == 'Rejected' && x.staffRoleType != 9 && x.supervisor != this.hrid);
