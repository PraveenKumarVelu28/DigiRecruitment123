...
@Component({
  selector: 'app-approver1ratingdashboard',
  templateUrl: './approver1ratingdashboard.component.html',
  styleUrls: ['./approver1ratingdashboard.component.css']
})
export class Approver1ratingdashboardComponent implements OnInit {
  ...
  public ConductappraisalStaffList() {
    this.fmService.GetConductappraisalStaffList().subscribe(
      res => {
        debugger;
        let temp: any = res
        this.StaffAppraisalList = temp.filter(x => x.approver1 == this.staffID && x.selfScores != null && x.groupHeadScores == null)
