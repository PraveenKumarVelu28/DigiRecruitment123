...
@Component({
  selector: 'app-work-place-request-completed',
  templateUrl: './work-place-request-completed.component.html',
  styleUrls: ['./work-place-request-completed.component.css']
})
export class WorkPlaceRequestCompletedComponent implements OnInit {
  ...
  public GetWorkSpaceDetails(SDate, EDate) {
    ...
    if (localStorage.getItem('roleid') == '4') {
      ...
      this.DigiOfficeService.GetWorkSpaceDetails(SDate, EDate).subscribe(data => {
        debugger;
        this.workSpaceRequest = data;
        this.filteredList = this.workSpaceRequest.filter(x => x.assignedTo == localStorage.getItem('staffID') &&  x.approvedStatus == 'Approved');
        document.getElementById('def_open').click();
        this.count = this.filteredList.length;
      });
    }
