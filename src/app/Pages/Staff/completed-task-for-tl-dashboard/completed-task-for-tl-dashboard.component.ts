...
@Component({
  selector: 'app-completed-task-for-tl-dashboard',
  templateUrl: './completed-task-for-tl-dashboard.component.html',
  styleUrls: ['./completed-task-for-tl-dashboard.component.css']
})
export class CompletedTaskForTlDashboardComponent implements OnInit {
  ...
  public GetTaskForDateWeb(StartDate,EndDate) {
    ...
    this.DigiOfficeService.GetTaskForDateWeb(StartDate,EndDate).subscribe(res => {
      debugger;
      this.taskList = res;
      this.filteredtaskList=this.taskList.filter(x=>x.supervisorID==this.managerID && x.status=="Completed");
