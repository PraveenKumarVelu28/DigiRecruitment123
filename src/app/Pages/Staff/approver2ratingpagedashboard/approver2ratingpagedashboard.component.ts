...
@Component({
  selector: 'app-approver2ratingpagedashboard',
  templateUrl: './approver2ratingpagedashboard.component.html',
  styleUrls: ['./approver2ratingpagedashboard.component.css']
})
export class Approver2ratingpagedashboardComponent implements OnInit {
  ...
  public ConductappraisalStaffList() {
    this.fmService.GetConductappraisalStaffList().subscribe(
      res => {
        debugger;
        let temp: any = res
        this.StaffAppraisalList = temp.filter(x => x.approver2 == this.staffID && x.selfScores != null && x.groupHeadScores != null && x.cioScores == null)
