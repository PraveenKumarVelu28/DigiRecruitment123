...
@Component({
  selector: 'app-rejected-hrstaff-leave-dashboard',
  templateUrl: './rejected-hrstaff-leave-dashboard.component.html',
  styleUrls: ['./rejected-hrstaff-leave-dashboard.component.css']
})
export class RejectedHRStaffLeaveDashboardComponent implements OnInit {
  ...
  public GetStaffLeaves(hrID,userRoleID,Sdate,Edate) {
    ...
    this.DigiOfficeService.GetStaffLeaves(hrID,userRoleID,Sdate,Edate).subscribe(res => {
      ...
      this.filteredrejectedStaffLeaveList=this.rejectedStaffLeaveList.filter(x=>x.status == 'Rejected' && x.supervisor ==this.hrID );
