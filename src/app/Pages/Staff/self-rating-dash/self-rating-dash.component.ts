...
@Component({
  selector: 'app-self-rating-dash',
  templateUrl: './self-rating-dash.component.html',
  styleUrls: ['./self-rating-dash.component.css']
})
export class SelfRatingDashComponent implements OnInit {
  ...
  public ConductappraisalStaffList() {
    this.fmService.GetConductappraisalStaffList().subscribe(
      res => {
        debugger;
        let temp: any = res
        this.StaffAppraisalList = temp.filter(x => x.id == this.staffID && x.selfScores == null);
        debugger
